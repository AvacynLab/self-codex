 ...
 */
export type ReviewKind = "code" | "text" | "plan" | "artifact";

/**
 * Description of an evaluation criterion provided by the caller. Each entry
 * may carry an optional weight to influence the aggregated score.
 */
export interface ReviewCriterion {
  /** Stable identifier (e.g. "clarity", "correctness"). */
  id: string;
  /** Optional human readable description shown in the breakdown. */
  description?: string;
  /** Importance applied when computing the global score (defaults to `1`). */
  weight?: number;
}

/**
 * Individual criterion evaluation returned by the meta critic.
 */
export interface ReviewBreakdownEntry {
  /** Identifier of the evaluated criterion. */
  criterion: string;
  /** Normalised score between `0` (poor) and `1` (excellent). */
  score: number;
  /** Concise explanation of the score. */
  reasoning: string;
}

/**
 * Result object produced by {@link MetaCritic.review}. The critic mirrors the
 * structure of quality assurance checklists used by human reviewers so the
 * orchestrator can automate gating decisions.
 */
export interface ReviewResult {
  /** Weighted average score across the supplied criteria. */
  overall: number;
  /** Qualitative verdict derived from {@link overall}. */
  verdict: "pass" | "warn" | "fail";
  /** Narrative feedback (bullet friendly) combining positives and negatives. */
  feedback: string[];
  /** Suggested corrections ordered by priority. */
  suggestions: string[];
...
/** Result returned by an individual heuristic. */
...
function fingerprintPayload(raw: string): string {
 ...
 */
function buildContext(output: string, kind: ReviewKind): EvaluationContext {
  const raw = output ?? "";
 ...
 */
function computeCompleteness(ctx: EvaluationContext): CriterionComputation {
  let score = 1;
 ...
 */
function computeClarity(ctx: EvaluationContext): CriterionComputation {
  let score = 1;
  ...
  if (averageSentenceLength > 35) {
    score -= 0.35;
    feedback.push(
      "Les phrases sont extrêmement longues (moyenne supérieure à 35 mots), ce qui nuit à la lisibilité.",
    );
    ...
    feedback.push(
      "Les phrases sont longues (moyenne supérieure à 22 mots), ce qui rend la lecture dense.",
    );
  ...
  if (ctx.kind === "code") {
    const indentationIssues = ctx.lines.filter(
      (line) => line.startsWith(" ") && (line.length - line.trimStart().length) % 2 === 1,
    );
 ...
 */
function computeCodeQuality(ctx: EvaluationContext): CriterionComputation {
  if (ctx.kind !== "code") {
 ...
 */
function computeTestingDiscipline(ctx: EvaluationContext): CriterionComputation {
 ...
 */
function computeRisk(ctx: EvaluationContext): CriterionComputation {
  let score = 1;
  const feedback: string[] = [];
  const suggestions: string[] = [];
 ...
 */
function defaultCriteriaFor(kind: ReviewKind): ReviewCriterion[] {
  if (kind === "code") {
    return [
