...
function detectAnchoring(steps: PlanStep[]): PlanBiasInsight | null {
  ...
  for (const [index, step] of steps.entries()) {
    const usesDomain =
      (step.domain && step.domain.toLowerCase() === normalisedDomain) ||
    ...
    const hasCounterTag =
      step.tags?.some((tag) => counterTags.has(tag.toLowerCase())) && index !== 0;
    if (hasCounterTag) {
      continue;
    }
  ...
  const severity: PlanBiasInsight["severity"] = ratio >= 0.75 ? "high" : "medium";
