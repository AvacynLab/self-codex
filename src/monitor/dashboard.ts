...
export interface DashboardSnapshot {
  ...
  /** Aggregated runtime cost metrics combining tokens and CPU latency. */
  runtimeCosts: DashboardRuntimeCosts;
  /**
  ...
  /** Ordered timeline of recent orchestrator events and filter metadata. */
  timeline: DashboardTimelineSnapshot;
  /** Consensus outcomes observed during recent planner joins. */
  consensus: DashboardConsensusSummary;
  /** Heatmap-ready ThoughtGraph projections grouped per job. */
  thoughtGraph: DashboardThoughtGraphHeatmap[];
...
export interface DashboardHeatmap {
  ...
  /** Aggregated CPU time per child based on elapsed runtime events. */
  latency: DashboardHeatmapCell[];
...
/** Aggregated runtime cost entry summarising the load for a child. */
export interface DashboardRuntimeCostEntry {
  /** Identifier of the child runtime. */
  childId: string;
  /** Human-friendly label derived from the child name when available. */
  label: string;
  /** Estimated tokens consumed by the child based on recent events. */
  tokens: number;
  /** Cumulative CPU/latency budget expressed in milliseconds. */
  latencyMs: number;
  /** Highest latency observed for a single event affecting the child. */
  maxLatencyMs: number;
}

/**
 * Aggregated runtime cost summary surfaced in dashboard snapshots. Operators
 * rely on the block to identify the most expensive children and to gauge the
 * overall compute footprint across the orchestrator.
 */
export interface DashboardRuntimeCosts {
  /** Total token usage observed across every child. */
  totalTokens: number;
  /** Cumulative latency across all events exposing elapsed milliseconds. */
  totalLatencyMs: number;
  /** Number of latency samples contributing to {@link totalLatencyMs}. */
  sampleCount: number;
  /** Average latency per event when samples have been recorded. */
  avgLatencyMs: number | null;
  /** Highest latency observed across every event. */
  maxLatencyMs: number | null;
  /** Child contributing the largest token usage, when available. */
  topTokenConsumer: DashboardRuntimeCostEntry | null;
  /** Child accumulating the largest latency budget, when available. */
...
/**
 * Timeline entry summarising a single orchestrator event. The dashboard uses the
 * structure to render a causal trace while keeping payload summaries concise and
 * safe for inline rendering.
 */
export interface DashboardTimelineEntry {
  /** Monotonic sequence assigned by the EventStore. */
  seq: number;
  /** Timestamp at which the event was recorded. */
  ts: number;
  /** Canonical event kind (PROMPT/STATUS/REPLY/…). */
  kind: EventKind;
  /** Severity associated with the event. */
  level: EventLevel;
  /** Origin of the event (orchestrator/child/system). */
  source: EventSource;
  /** Job identifier when available. */
  jobId: string | null;
  /** Child identifier when available. */
  childId: string | null;
  /** Short human-friendly summary extracted from the payload. */
  summary: string;
}

/** Distinct filter values exposed to the timeline dropdowns. */
export interface DashboardTimelineFilters {
  kinds: string[];
  levels: string[];
  sources: string[];
  jobs: string[];
...
/** Timeline snapshot combining ordered events and the available filters. */
export interface DashboardTimelineSnapshot {
  events: DashboardTimelineEntry[];
  filters: DashboardTimelineFilters;
}

/**
 * Canonical summary of a consensus decision extracted from plan join/status
 * events. The structure keeps dashboard rendering logic independent from the
 * exact payload shape emitted by coordination utilities.
 */
export interface DashboardConsensusDecision {
  seq: number;
  ts: number;
  jobId: string | null;
  childId: string | null;
...
/** Aggregated counters and latest entries for the consensus dashboard panel. */
export interface DashboardConsensusSummary {
  /** Most recent decision, or `null` when the orchestrator never voted. */
  latest: DashboardConsensusDecision | null;
  /** Recent history ordered from newest to oldest, capped for readability. */
  recent: DashboardConsensusDecision[];
  /** Quick statistics surfaced above the table. */
  stats: {
    total: number;
    satisfied: number;
    unsatisfied: number;
...
/**
 * Individual reasoning branch surfaced in the ThoughtGraph heatmap. Normalised
 * scores allow the dashboard to render gradients without recomputing bounds on
 * the client.
 */
export interface DashboardThoughtGraphNode {
  id: string;
  depth: number;
...
/** Grouped ThoughtGraph snapshot per job for the heatmap visualisation. */
export interface DashboardThoughtGraphHeatmap {
  jobId: string;
  goal: string | null;
  updatedAt: number;
  nodes: DashboardThoughtGraphNode[];
}
 ...
 */
function buildDashboardTimeline(eventStore: EventStore, limit = 40): DashboardTimelineSnapshot {
  const recent = eventStore.list({ limit, reverse: true }).reverse();
  const events: DashboardTimelineEntry[] = recent.map((event) => ({
    seq: event.seq,
    ts: event.ts,
    kind: event.kind,
    level: event.level,
...
function buildTimelineFilters(events: DashboardTimelineEntry[]): DashboardTimelineFilters {
...
function summariseEventForTimeline(event: OrchestratorEvent): string {
  ...
  if (!payload || typeof payload !== "object") {
    ...
    if (tokens > 0 || (latency ?? 0) > 0) {
      ...
      if (tokens > 0) fragments.push(`tokens=${tokens}`);
      if (latency !== null && latency > 0) fragments.push(`latency=${latency}ms`);
  ...
  if ("consensus" in payload && typeof payload.consensus === "object" && payload.consensus !== null) {
    ...
    if (satisfied !== null) parts.push(satisfied ? "ok" : "ko");
 ...
 * panel.
 */
function summariseConsensusDecisions(
  eventStore: EventStore,
  limit = 10,
): DashboardConsensusSummary {
  const decisions: DashboardConsensusDecision[] = [];
...
function buildThoughtGraphHeatmap(graphState: GraphState): DashboardThoughtGraphHeatmap[] {
  const jobs = graphState.listJobs();
  const projections: DashboardThoughtGraphHeatmap[] = [];
  ...
  for (const job of jobs) {
    ...
    const nodes = snapshot.nodes.map<DashboardThoughtGraphNode>((node) => {
...
function extractConsensusPayload(value: Record<string, unknown>): Record<string, unknown> | null {
...
function collectConsensusMetadata(
  ...
  if (typeof consensus.metadata === "object" && consensus.metadata !== null) {
    for (const [key, value] of Object.entries(consensus.metadata as Record<string, unknown>)) {
      if (!(key in metadata)) {
        metadata[key] = value;
      }
    }
...
function normaliseConsensusTally(value: unknown): Record<string, number> | null {
...
function normaliseThoughtScore(
...
function truncateJson(record: Record<string, unknown>, maxLength: number): string {
  try {
...
function extractLatencyMs(event: OrchestratorEvent): number | null {
  const payload = event.payload as Record<string, unknown> | null | undefined;
  if (!payload || typeof payload !== "object") {
    return null;
  }
  ...
  const normalised =
    extractLatencyCandidate(payload) ??
    (typeof payload.metrics === "object" && payload.metrics !== null
      ? extractLatencyCandidate(payload.metrics as Record<string, unknown>)
      : null);
...
function extractLatencyCandidate(record: Record<string, unknown>): number | null {
  const keys = [
    "elapsed_ms",
    "elapsedMs",
    "duration_ms",
...
export function computeDashboardHeatmap(
  ...
  const latencyUsage = new Map<string, number>();
  ...
  const latency: DashboardHeatmapCell[] = [];
  for (const child of children) {
 ...
 */
export function summariseRuntimeCosts(
  graphState: GraphState,
  eventStore: EventStore,
): DashboardRuntimeCosts {
  const labels = new Map<string, string>();
  for (const child of graphState.listChildSnapshots()) {
    labels.set(child.id, child.name ?? child.id);
  }

  const perChild = new Map<string, { tokens: number; latencyMs: number; maxLatencyMs: number }>();
  let totalTokens = 0;
  let totalLatencyMs = 0;
  let sampleCount = 0;
  let maxEventLatency: number | null = null;
  ...
  const perChildEntries: DashboardRuntimeCostEntry[] = [];
  let topToken: DashboardRuntimeCostEntry | null = null;
  let topLatency: DashboardRuntimeCostEntry | null = null;

  for (const [childId, stats] of perChild.entries()) {
    if (stats.tokens <= 0 && stats.latencyMs <= 0) {
      continue;
    }
    const entry: DashboardRuntimeCostEntry = {
      childId,
...
function renderHeatmapCellsHtml(cells: DashboardHeatmapCell[], emptyMessage: string): string {
  if (cells.length === 0) {
  ...
  const rendered = normalised
    .map(({ cell, normalised }) => {
      ...
      return `<span class="heatmap-cell" style="--cell-alpha:${alpha}" title="${escapeHtml(
        `${cell.label} – ${valueLabel}`,
      )}"><strong>${escapeHtml(cell.label)}</strong><small>${escapeHtml(valueLabel)}</small></span>`;
    })
    .join("\n");
...
function normaliseHeatmapCellsForHtml(
  cells: DashboardHeatmapCell[],
): Array<{ cell: DashboardHeatmapCell; normalised: number }> {
  let fallbackMax = 0;
...
function renderTimelineFilterOptions(values: string[]): string {
...
function renderTimelineEventsHtml(events: DashboardTimelineEntry[]): string {
  if (events.length === 0) {
  ...
  return events
    .map((event) => {
      ...
      return `<li><time${datetimeAttr}>${escapeHtml(formatTimestamp(event.ts))}</time><span class="timeline-kind">${escapeHtml(
        event.kind,
      )}</span><span class="timeline-summary">${escapeHtml(event.summary)}</span><span class="timeline-meta">${escapeHtml(
        metaLabel,
      )}</span></li>`;
    })
    .join("\n");
...
function renderConsensusStatsHtml(stats: DashboardConsensusSummary["stats"]): string {
...
function renderConsensusLatestHtml(latest: DashboardConsensusDecision | null): string {
  if (!latest) {
  ...
  if (latest.metadata) {
    ...
    if (typeof latest.metadata.quorum_threshold === "number") {
      details.push(`Quorum : ${formatNumber(latest.metadata.quorum_threshold as number)}`);
    }
    if (details.length > 0) {
...
function formatConsensusBoolean(value: boolean | null): string {
  if (value === null) {
    return "n/a";
  }
...
function formatConsensusTally(tally: Record<string, number>): string {
...
function renderConsensusHistoryHtml(decisions: DashboardConsensusDecision[]): string {
  ...
  const rows = decisions
    .map((decision) => {
      ...
      const datetimeAttr = iso ? ` datetime=\"${escapeHtml(iso)}\"` : "";
      return `<tr><td><time${datetimeAttr}>${escapeHtml(formatTimestamp(decision.ts))}</time></td><td>${escapeHtml(
        decision.mode ?? "n/a",
      )}</td><td>${escapeHtml(decision.outcome ?? "n/a")}</td><td>${escapeHtml(
        decision.votes === null ? "n/a" : formatNumber(decision.votes),
      )}</td><td>${escapeHtml(formatConsensusBoolean(decision.tie))}</td></tr>`;
    })
    .join("\n");
...
function formatThoughtGoal(goal: string | null): string {
  if (!goal) {
    return "Objectif : n/a";
  }
...
function renderThoughtGraphHeatmapHtml(projections: DashboardThoughtGraphHeatmap[]): string {
    ...
    .map((projection) => {
      const nodes = projection.nodes
        .map((node) => {
          ...
          const details = `Score : ${scoreLabel} • Profondeur : ${node.depth} • Statut : ${node.status}${
            node.provenanceCount > 0 ? ` • Provenance : ${node.provenanceCount}` : ""
          }`;
          return `<span class="thought-heatmap-node thought-heatmap-node--${escapeHtml(
            node.status,
          )}" style="--node-alpha:${alpha}" title="${escapeHtml(details)}"><strong>${escapeHtml(
            node.label,
          )}</strong><small>${escapeHtml(details)}</small></span>`;
        })
        .join("\n");
      return `<article class="thought-heatmap-card"><header><h3>${escapeHtml(
        projection.jobId,
      )}</h3><p>${escapeHtml(formatThoughtGoal(projection.goal))}</p><p class="thought-heatmap-meta">Mise à jour : ${escapeHtml(
        formatTimestamp(projection.updatedAt),
      )}</p></header><div class="thought-heatmap-nodes">${nodes}</div></article>`;
    })
    .join("\n");
}

/** Converts a timestamp to ISO format while guarding against invalid values. */
function formatIsoTimestamp(value: number): string | null {
...
function renderDashboardHtml(snapshot: DashboardSnapshot): string {
  ...
  const runtimeSummaryRows: Array<[string, string]> = [
  ...
  const runtimeSummary = renderMetricsTableHtml(
    runtimeSummaryRows,
    "Aucune métrique runtime disponible pour le moment.",
  );

  const runtimeLeaderboardRows = snapshot.runtimeCosts.perChild.slice(0, 5).map<[string, string]>((entry) => [
    `${entry.label} (${entry.childId})`,
    `Tokens : ${formatNumber(entry.tokens)} · CPU : ${formatNumber(entry.latencyMs)} ms`,
  ]);
  const runtimeLeaderboard = renderMetricsTableHtml(
    runtimeLeaderboardRows,
    "Aucun enfant actif n'a encore publié de métriques.",
  );

  const heatmapIdle = renderHeatmapCellsHtml(
    snapshot.heatmap.idle,
    "Aucun enfant ne semble inactif pour le moment.",
  );
  const heatmapErrors = renderHeatmapCellsHtml(
    snapshot.heatmap.errors,
    "Aucune erreur n'a été recensée récemment.",
  );
  const heatmapTokens = renderHeatmapCellsHtml(
    snapshot.heatmap.tokens,
    "Aucune consommation de tokens n'a été enregistrée.",
  );
  const heatmapLatency = renderHeatmapCellsHtml(
    snapshot.heatmap.latency,
    "Aucune mesure de latence n'est disponible.",
  );
  const heatmapPheromones = renderHeatmapCellsHtml(
    snapshot.heatmap.pheromones,
    "Aucune phéromone n'a encore été déposée dans le champ.",
  );
...
/** Result returned when parsing a log tail query. */
interface ParsedLogTailQuery {
  stream: LogStream;
...
type LogTailQueryParseResult =
  | { ok: true; query: ParsedLogTailQuery }
  | { ok: false; message: string; issues: readonly string[] };

/** Normalises query parameters supplied to `GET /logs`. */
function parseLogTailQuery(params: URLSearchParams): LogTailQueryParseResult {
...
function collectQueryStrings(params: URLSearchParams, ...keys: string[]): string[] {
...
function normaliseLogStream(value: string): LogStream | null {
...
interface OptionalIntParseResult {
  value: number | null;
  error?: string;
}
...
interface LevelParseResult {
  value: readonly string[] | null;
  error?: string;
}

/** Normalises severity filters into canonical logger levels. */
function normaliseLevelFilters(values: string[]): LevelParseResult {
  if (values.length === 0) {
    return { value: null };
  }
