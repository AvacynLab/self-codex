...
import {
  ...
  suggestPlanFragments,
  type KnowledgeAssistResult,
  type KnowledgeAssistRagEvidence,
  type PlanAssistContext,
  type PlanAssistSuggestion,
} from "../knowledge/assist.js";
import { StructuredLogger } from "../logger.js";
import type { HybridRetriever, HybridRetrieverSearchOptions } from "../memory/retriever.js";
import {
  PROVENANCE_TYPES,
...
export interface KnowledgeToolContext {
  ...
  /** Optional RAG components used when assisting knowledge queries. */
  rag?: {
    /** Lazily resolves the retriever used for fallback when KG coverage is low. */
    getRetriever: () => Promise<HybridRetriever | null>;
    /** Default domain tags enforced on RAG lookups. */
    defaultDomainTags?: string[];
    /** Minimum cosine score enforced when the caller omits an explicit threshold. */
    minScore?: number;
  };
}
...
/** Result returned by {@link handleKgAssist}. */
export interface KgAssistResult extends KnowledgeAssistResult {}
  ...
  const inserted = input.triples.map((triple) => {
    ...
    if (triple.provenance !== undefined) {
      const provenance = normaliseProvenanceList(
        triple.provenance as ReadonlyArray<Provenance | null | undefined>,
      );
 ...
 */
function exportKnowledgeForRag(
  knowledgeGraph: KnowledgeGraph,
  options: KnowledgeRagExportOptions,
): KnowledgeRagDocument[] {
...
export async function handleKgSuggestPlan(
  context: KnowledgeToolContext,
  input: z.infer<typeof KgSuggestPlanInputSchema>,
): Promise<KgSuggestPlanResult> {
  ...
  let planContext: PlanAssistContext | undefined;
  if (input.context) {
    const overrides: PlanAssistContext = {};
  ...
  if (ragConfig?.getRetriever) {
    ...
    if (retriever) {
      ...
      if (needsPlanRagFallback(suggestion)) {
        ...
        const augmented: PlanAssistSuggestion = {
          ...suggestion,
          coverage,
  ...
  const result: PlanAssistSuggestion = { ...suggestion, coverage };
...
export async function handleKgAssist(
  context: KnowledgeToolContext,
  input: z.infer<typeof KgAssistInputSchema>,
): Promise<KgAssistResult> {
  ...
  const resolvedMinScore = Math.max(
    context.rag?.minScore ?? 0,
    Number.isFinite(input.min_score) ? input.min_score : 0,
  );
  ...
  } satisfies Parameters<typeof assistKnowledgeQuery>[1];
...
function needsPlanRagFallback(suggestion: PlanAssistSuggestion): boolean {
...
interface PlanRagFallbackContext {
  retriever: HybridRetriever;
  goal: string;
  suggestion: PlanAssistSuggestion;
  domainTags: string[];
  minScore: number;
...
interface PlanRagFallbackResult {
  evidence: KnowledgeAssistRagEvidence[];
  query: string;
  domainTags: string[];
  minScore: number;
}
 ...
 */
async function collectPlanRagFallback(context: PlanRagFallbackContext): Promise<PlanRagFallbackResult> {
  ...
  const evidence: KnowledgeAssistRagEvidence[] = hits.map((hit) => ({
    id: hit.id,
...
function tokeniseForRag(text: string): string[] {
  return text
    .toLowerCase()
    .split(/[^\p{L}\p{N}]+/u)
...
function computeMatchedTerms(queryTokens: string[], text: string): string[] {
...
function truncatePlanRagSnippet(text: string): string {
...
function roundScore(value: number, decimals = 4): number {
...
function normaliseDomainTags(raw: Array<string | undefined>): string[] {
  if (!raw.length) {
    return [];
  }
