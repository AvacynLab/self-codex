 ...
 */
export function analysePlanBias(steps, options = {}) {
...
function detectAnchoring(steps) {
    if (steps.length < 3) {
        return null;
    }
...
function detectMonoculture(steps, options) {
    if (steps.length === 0) {
        return null;
    }
    ...
    const severity = dominantRatio >= 0.8 ? "high" : "medium";
    return {
        type: "monoculture",
        severity,
        explanation: "Les étapes partagent les mêmes expertises ou domaines, laissant peu de place à la diversité des approches.",
        recommendedActions: [
...
function detectMissingExploration(steps, options) {
...
function detectRiskBlindness(steps, options) {
    if (steps.length === 0) {
        return null;
    }
