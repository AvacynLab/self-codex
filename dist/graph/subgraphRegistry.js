 ...
 */
export function parseSubgraphRegistry(value) {
 ...
 */
export function stringifySubgraphRegistry(registry) {
    const serialisable = {};
 ...
 */
export function collectSubgraphReferences(graph) {
    const refs = new Set();
    for (const node of graph.nodes) {
        ...
        const ref = typeof attributes.ref === "string"
            ? attributes.ref
 ...
 */
export function collectMissingSubgraphDescriptors(graph) {
 ...
 */
export function resolveSubgraphDescriptor(graph, ref) {
    const metadata = graph.metadata;
    const registry = parseSubgraphRegistry(metadata?.[SUBGRAPH_REGISTRY_KEY]);
    if (!registry) {
        return null;
