...
function pushEvent(e) {
    ...
    if (GLOBAL_EVENTS.length > MAX_EVENTS_GLOBAL)
        GLOBAL_EVENTS.shift();
    if (evt.jobId) {
        const arr = JOB_EVENTS.get(evt.jobId) ?? [];
        arr.push(evt);
        if (arr.length > MAX_EVENTS_PER_JOB)
            arr.shift();
...
server.registerTool("graph_state_inactivity", {
    ...
    if (defaultFormat === "text") {
        ...
        const lines = limited.map((report) => {
            ...
            return `- ${report.childId} (${report.state}) job=${job} runtime=${report.runtime} idle=${idleSec ?? "∅"}s pending=${pendingSec ?? "∅"}s waiting=${waiting} flags=[${flagSummary}]`;
        });
...
// plan_fanout
server.registerTool("plan_fanout", { title: "Plan fan-out", description: "Planifie des enfants en parallele, retourne job_id et child_ids.", inputSchema: PlanFanoutShape }, async (input) => {
    pruneExpired();
    const { goal, children = [], ttl_s, max_children = 6 } = input;
    let planned = children.map((child) => ({ ...child, runtime: child.runtime ?? "codex" }));
