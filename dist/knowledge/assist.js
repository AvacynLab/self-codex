 ...
 */
export async function assistKnowledgeQuery(knowledgeGraph, options) {
    ...
    const knowledgeEvidence = collectKnowledgeEvidence(knowledgeGraph.exportAll(), scoringTokens, queryTokens, limit);
    const knowledgeMatchedTerms = new Set();
    ...
    if (options.ragRetriever && knowledgeEvidence.length < limit) {
        ragEvidence = await collectRagEvidence(options.ragRetriever, query, context, queryTokens, ragLimit, ragMinScore, domainTags);
    }
    const matchedTerms = new Set(knowledgeMatchedTerms);
...
function computeLexicalScore(queryTokens, text) {
...
function collectKnowledgeEvidence(triples, scoringTokens, queryTokens, limit) {
    if (triples.length === 0) {
        return [];
    }
    const candidates = [];
...
function collectKnowledgeProvenance(triple) {
    ...
    const fallback = { sourceId: `kg:${triple.id}`, type: "kg" };
...
function aggregateCitations(knowledge, rag) {
    let citations = [];
...
function buildAssistAnswer(knowledge, rag) {
    const sections = [];
...
function buildAssistRationale(knowledge, rag, totalQueryTokens, matchedTokens, domainTags, ragEnabled) {
    const rationale = [];
    if (knowledge.length > 0) {
        const avgConfidence = knowledge.reduce((acc, entry) => acc + entry.triple.confidence, 0) / knowledge.length;
        rationale.push(`Connaissances mobilisées : ${knowledge.length} fait(s) (confiance moyenne ${round(avgConfidence, 3)}).`);
    }
    else {
